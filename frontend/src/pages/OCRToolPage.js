...
//     if (!response) return '분석 결과가 없습니다.';
    
//     try {
//       // 이미지/텍스트 분석 결과 구분
//       if (typeof response === 'string' && 
//           response.includes("이미지 분석 결과:") && 
//           response.includes("텍스트 분석 결과:")) {
        
//         // 안전하게 문자열 분리
//         const parts = response.split("텍스트 분석 결과:");
//         const imageAnalysisPart = parts[0] || '';
//         const textAnalysisPart = parts[1] || '';
        
//         // 각 부분 정리
//         const imageAnalysis = imageAnalysisPart.replace("이미지 분석 결과:", "").trim();
//         const textAnalysis = textAnalysisPart.trim();
        
//         return (
//           <div>
//             <div className="mb-4">
//               <h4 className="text-md font-medium text-gray-900 mb-2">이미지 내용 분석</h4>
//               <div className="bg-blue-50 p-3 rounded border border-blue-200 whitespace-pre-wrap">
//                 {imageAnalysis || "이미지 분석 결과가 없습니다."}
//               </div>
//             </div>
//             <div>
//               <h4 className="text-md font-medium text-gray-900 mb-2">텍스트 분석</h4>
//               <div className="bg-green-50 p-3 rounded border border-green-200 whitespace-pre-wrap">
//                 {textAnalysis || "텍스트 분석 결과가 없습니다."}
//               </div>
//             </div>
//           </div>
//         );
...
//       // PDF 페이지별 분석 결과 처리
//       if (typeof response === 'string' && response.includes("PDF 페이지별 분석 결과:")) {
//         // 페이지별 분석과 전체 요약 분리
//         const parts = response.split("전체 문서 요약:");
//         const pagesAnalysis = parts[0] || '';
//         const summary = parts[1] || '';
        
//         // 페이지별 구분
//         const pageSegments = pagesAnalysis.split(/---\s*페이지\s*\d+\s*분석\s*---/)
//           .filter(segment => segment.trim().length > 0);
        
//         // 첫 번째 부분은 헤더이므로 제거하거나 처리
//         const headerText = pageSegments.shift();
        
//         return (
//           <div>
//             <div className="mb-4">
//               <h4 className="text-md font-medium text-gray-900 mb-2">PDF 페이지별 분석</h4>
//               {pageSegments.map((segment, index) => (
//                 <div key={index} className="mb-4">
//                   <h5 className="text-sm font-medium text-gray-800 mb-1">페이지 {index + 1} 분석</h5>
//                   <div className="bg-blue-50 p-3 rounded border border-blue-200 whitespace-pre-wrap">
//                     {segment.trim() || "분석 결과가 없습니다."}
//                   </div>
//                 </div>
//               ))}
//             </div>
...
//             {summary && (
//               <div>
//                 <h4 className="text-md font-medium text-gray-900 mb-2">전체 문서 요약</h4>
//                 <div className="bg-green-50 p-3 rounded border border-green-200 whitespace-pre-wrap">
//                   {summary.trim() || "요약 결과가 없습니다."}
//                 </div>
//               </div>
//             )}
//           </div>
...
// const parsePagedAnalysis = (analysisText) => {
//   if (!analysisText) return [];
  
//   // 페이지 구분 패턴 (더 정확한 패턴 정의)
//   const pageHeaderPattern = /={3,5}\s*페이지\s*(\d+(?:-\d+)?)\s*(?:분석)?\s*={3,5}/gi;
...
//   if (matches.length === 0) {
//     // "**📄 페이지 X**" 패턴 확인
//     const altPattern = /\*\*📄\s*페이지\s*(\d+(?:-\d+)?)\*\*/gi;
...
//     // 중복 패턴 제거
//     content = content.replace(/\*\*📄\s*페이지\s*\d+(?:-\d+)?\*\*/gi, "");
//     content = content.replace(/페이지\s*\d+(?:-\d+)?$/gm, "");
...
//   }
//   // 전체 요약 섹션 찾기
//   const summaryPattern = /={3,5}\s*전체\s*(?:문서)?\s*(?:요약|정리)\s*={3,5}/i;
...
//   // 이미지와 텍스트 분석 결과 분리
//   const parseImageAndTextAnalysis = (analysisText) => {
//     if (!analysisText) return { imageAnalysis: "", textAnalysis: "" };
    
//     const imageSectionMatch = analysisText.match(/이미지\s*분석\s*결과:[\s\S]*?(?=텍스트\s*분석\s*결과:|$)/i);
//     const textSectionMatch = analysisText.match(/텍스트\s*분석\s*결과:[\s\S]*/i);
    
//     const imageAnalysis = imageSectionMatch 
//       ? imageSectionMatch[0].replace(/이미지\s*분석\s*결과:/i, "").trim()
//       : "";
      
//     const textAnalysis = textSectionMatch
//       ? textSectionMatch[0].replace(/텍스트\s*분석\s*결과:/i, "").trim()
//       : "";
...
//     if (!result || !result.llm_response) return null;
    
//     // 이미지 파일인 경우 이미지/텍스트 분석을 구분
//     if (result.file_type === 'image') {
//       const { imageAnalysis, textAnalysis } = parseImageAndTextAnalysis(result.llm_response);
      
//       return (
//         <div className="space-y-6">
//           {imageAnalysis && (
//             <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
//               <h3 className="text-lg font-medium text-blue-900 mb-2">이미지 분석 결과</h3>
...
//               <h3 className="text-lg font-medium text-purple-900 mb-2">텍스트 분석 결과</h3>
//               <div className="prose prose-purple max-w-none">
//                 {parsePagedAnalysis(textAnalysis).map((page, idx) => (
...
//                       {page.pageNum === "전체 요약" ? "전체 요약" : `페이지 ${page.pageNum}`}
//                     </h4>
//                     <div>
//                       {page.content.split("\n").map((line, lineIdx) => (
...
//       const pages = parsePagedAnalysis(result.llm_response);
      
//       return (
//         <div className="space-y-4">
//           <div className="flex justify-between items-center mb-2">
//             <h3 className="text-lg font-medium text-gray-900">페이지별 분석 결과</h3>
//             {pages.length > 1 && (
//               <span className="text-sm text-gray-500">총 {pages.length}개 섹션</span>
//             )}
//           </div>
...
//                   }`}>
//                     {page.pageNum === "전체 요약" ? "📋 전체 문서 요약" : `📄 페이지 ${page.pageNum}`}
//                   </h4>
...
//                       textRelevant={result.text_relevant} // 백엔드에서 전달받은 텍스트 관련성 정보
//                     />
//                   )}
                  
//                   {/* LLM 분석 - Ollama나 Both 모드일 때만 표시 */}
//                   {(analysisType === "ollama" || analysisType === "both") && result.llm_response && (
//                     <div className="bg-gray-50 p-4 rounded-md mb-6">
//                       <h3 className="text-lg font-medium text-gray-900 mb-2">Ollama 분석 결과</h3>
//                       <div className="bg-white p-3 rounded border border-gray-200">
//                         <div className="prose prose-sm max-w-none text-gray-800">
//                           {result.file_type === 'pdf'
//                             ? renderAnalysisResult()
//                             : formatLLMResponse(result.llm_response)}
//                         </div>
//                       </div>
//                     </div>
//                   )}
